...
% ---------------- Problem 5 ----------------
\problem{Half and Full Quantum Adder: Implementation and Interpretation}{20}
\begin{summary}
In this problem you will connect provided circuit diagrams for a 1-bit half adder and full adder (with carry) to its behavior (see Figure 1 and 2). You will implement the circuits (which is made of of CNOT, Toffoli (Controlled CNOT), reason about one input by hand, and then validate all input cases on a simulator. Measure S the sum and C the carry for given inputs. A half adder is an add that only calculates the sum and the carry for a stand alone add - not additions that can be strung together. However, a full adder can do chain addition to do "ripple-carry adders".
\begin{figure}[h!]
  \centering
  \includegraphics[width=\textwidth]{halfadder.png}
  \caption{Half Quantum Adder (without ripple(chain)-carry)}
...
\begin{figure}[h!]
  ...
  \caption{Full Quantum Adder (with ripple(chain)-carry)}
  \label{fig:your_label1}
\end{figure}

\begin{enumerate}[label=(\alph*)]

  \item Work out by hand what happens to $\ket{110}$ as input to the half quantum adder and $\ket{1100}$ for the full adder. What is the final expect quantum state before measurement.
  \item Implement both of the circuits circuit in Qiskit, initializing carry to $\ket{0}$ and setting inputs with $X$ gates as needed.
  \item Run for inputs $(q_0,q_1)\in\{00,01,10,11\}$ for the half adder. Do the same with the full adder, with $C=q3=0$. Use conditional $X$ gates to prepare inputs that start in the ground state on the appropiate qubit. Record outputs $(q_1,q_2)$ which represent the value of the addition and if carry is present, respectively for the half adder, and measure $(q_3,q_4)$ for the full adder.
